<template>
  ...
  <div class="row">
    <h4 class="text-center text-primary font-weight-bold">
      <i class="bi bi-person-lines-fill mr-2"></i> GESTIÓN DE USUARIOS
    ...
    <form class="form-in-line">
      <div class="col-10 col-m-6 col-lg-8 mx-auto">
        ...
        <div class="input-group-text mb-3">
          ...
          <input type="email" class="form-control sm w-50" placeholder="Email" v-model="usuario.email" @blur="validarEmail(this.usuario.email)">
...
export default {
  ...
  if (!regex.test(email)) {
    ...
    async grabarusuario() {
      ...
      if (this.usuario.dni && this.usuario.apellidos && this.usuario.telefono && this.usuario.alta && this.usuario.direccion
        && this.usuario.municipio && this.usuario.provincia && this.usuario.tipo && this.usuario.nombre
      ) {
        try {
          this.usuario.baja = '';
  ...
  const month = String(hoy.getMonth() + 1).padStart(2, '0'); // Asegurar que el mes tenga 2 dígitos
  const day = String(hoy.getDate()).padStart(2, '0'); // Asegurar que el día tenga 2 dígitos
  return `${year}-${month}-${day}`;
    ...
    async grabarOmodificarUsuario() {
  // Verificar si los campos requeridos están llenos
  if (this.usuario.dni && this.usuario.apellidos && this.usuario.telefono) {
    try {
      // Establecer el valor de 'baja' si es necesario
      this.usuario.baja = '';

      // Si el usuario tiene un 'id', es una actualización, si no es un nuevo usuario
      if (this.usuario.id) {
        // Confirmar si el usuario realmente quiere actualizar
        ...
        if (confirmar) {
          ...
          this.mostrarAlerta("Aviso", "La modificación del usuario fue cancelada", "info");
        }
      } else {
        // Si no tiene 'id', significa que es un nuevo usuario, crear el usuario
        const response = await fetch('http://localhost:3000/usuarios', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      console.error(error);
      this.mostrarAlerta('Error', 'No se pudo grabar o modificar el usuario.', 'error');
    }
  } else {
    this.mostrarAlerta('Error', 'Por favor, completa todos los campos requeridos.', 'error');
  }
